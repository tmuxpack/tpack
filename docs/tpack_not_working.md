# Help, tpack not working!

Here's the list of issues users had with `tpack`:

<hr />

> Nothing works. `tpack` key bindings `prefix + I`, `prefix + U` not even
  defined.

Related [issue #22](https://github.com/tmux-plugins/tpm/issues/22)

- Do you have required `tmux` version to run `tpack`?<br/>
  Check `tmux` version with `$ tmux -V` command and make sure it's higher or
  equal to the required version for `tpack` as stated in the readme.

- ZSH tmux plugin might be causing issues.<br/>
  If you have it installed, try disabling it and see if `tpack` works then.
...
`tpack` has a known issue when using custom config file with `-f` option.
...
3. Reload TMUX environment so tpack is sourced: `$ tmux source /path/to/my_tmux.conf`
...
> tpack not working: '~/.tmux/plugins/tpm/tpm' returned 2 (Windows / Cygwin)
